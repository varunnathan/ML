...
def runET(train_X, train_y, test_X, test_y=None, test_X2=None, depth=20,
          leaf=10, feat=0.2):
    ...
    for model_name in ["LGB1", "XGB1"]:
        ...
        for c in train_df.columns:
            if train_df[c].dtype == 'object' and c not in ["Responders",
                ...
                lbl.fit(list(train_df[c].values.astype('str')) + list(
                 test_df[c].values.astype('str')))
                train_df[c] = lbl.transform(list(train_df[c].values.astype(
                 'str')))
                test_df[c] = lbl.transform(list(test_df[c].values.astype(
                 'str')))
        ...
        f1_f2_list = [["D_prev1", "D_prev2"], ["D_prev2", "D_prev3"],
                      ["D_prev3", "D_prev4"], ["D_prev4", "D_prev5"],
                      ["D_prev5", "D_prev6"],
                      ["CR_AMB_Prev1", "CR_AMB_Prev3"],
                      ["CR_AMB_Prev1", "CR_AMB_Prev4"],
                      ["CR_AMB_Prev1", "CR_AMB_Prev5"],
                      ["CR_AMB_Prev1", "CR_AMB_Prev6"],
                      ["EOP_prev1", "CR_AMB_Prev1"],
                      ["EOP_prev2", "CR_AMB_Prev2"],
                      ["EOP_prev3", "CR_AMB_Prev3"],
                      ["EOP_prev4", "CR_AMB_Prev4"],
                      ["EOP_prev5", "CR_AMB_Prev5"],
                      ["EOP_prev6", "CR_AMB_Prev6"],
                      ["EOP_prev1", "EOP_prev2"], ["EOP_prev2", "EOP_prev3"],
                      ["EOP_prev3", "EOP_prev4"], ["EOP_prev4", "EOP_prev5"],
                      ["EOP_prev5", "EOP_prev6"],
                      ["CR_AMB_Prev2", "CR_AMB_Prev4"],
                      ["CR_AMB_Prev2", "CR_AMB_Prev5"],
                      ["CR_AMB_Prev2", "CR_AMB_Prev6"],
                      ["EOP_prev1", "CR_AMB_Prev2"],
                      ["EOP_prev1", "CR_AMB_Prev3"],
                      ["EOP_prev1", "CR_AMB_Prev4"],
                      ["EOP_prev1", "CR_AMB_Prev5"],
                      ["EOP_prev1", "CR_AMB_Prev6"],
                      ["CR_AMB_Drop_Build_1", "CR_AMB_Drop_Build_2"],
                      ["CR_AMB_Drop_Build_2", "CR_AMB_Drop_Build_3"],
                      ["CR_AMB_Drop_Build_3", "CR_AMB_Drop_Build_4"],
                      ["BAL_prev1", "BAL_prev2"], ["BAL_prev2", "BAL_prev3"],
                      ["BAL_prev3", "BAL_prev4"],
                      ["BAL_prev1", "CR_AMB_Prev1"],
                      ["BAL_prev2", "CR_AMB_Prev2"],
                      ["BAL_prev3", "CR_AMB_Prev3"],
                      ["I_AQB_PrevQ1", "I_AQB_PrevQ2"],
                      ["I_NRV_PrevQ1", "I_NRV_PrevQ2"],
                      ["D_prev1", "D_prev3"], ["D_prev1", "D_prev4"],
                      ["D_prev1", "D_prev6"],
                      ]
        ...
        for dev_index, val_index in kf.split(train_X):
            ...
            if model_name == "XGB1":
                pred_val, loss, pred_test = runXGB(
                 dev_X, dev_y, val_X, val_y, test_X, rounds=5000, dep=8,
        ...
        out_df.to_csv("./meta_models/test/pred_test_v5_"+model_name+".csv",
                      index=False)

        out_df = pd.DataFrame({"UCIC_ID": train_id})
        out_df["Responders"] = pred_val_full
        out_df.to_csv("./meta_models/val/pred_val_v5_"+model_name+".csv",
                      index=False)
